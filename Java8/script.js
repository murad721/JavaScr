...
function showPeople(team) {
    main_container.innerHTML= "";
    people = JSON.parse(localStorage.getItem(team));
    for (let i = 0; i < people.length; i++) {
        const div = document.createElement("div");
        div.className = "inner-container";

        const h2 = document.createElement("h2");
        h2.innerText = people[i].fullname;

        const p1 = document.createElement("p");
        p1.innerText = "Age: " + people[i].age;

        const p2 = document.createElement("p");
        p2.innerText = "Score: " + people[i].average_score;

        const p3 = document.createElement("p");
        p3.innerText = "Grade: " + people[i].grade;
